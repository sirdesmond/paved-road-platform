...
Kubebuilder generates most of this. You only hand-edit two files:

```
paved-road-platform/
└── environment-controller/                          # NEW (kubebuilder scaffolds it)
    ├── api/v1alpha1/environment_types.go            ← you edit this
    ├── api/v1alpha1/groupversion_info.go            (generated — read it, don't edit)
    ├── internal/controller/environment_controller.go (generated — example 02)
    ├── config/crd/bases/…_environments.yaml         (generated by `make manifests`)
    ├── config/samples/platform_v1alpha1_environment.yaml  ← you edit this
    └── Makefile                                     (generated)
```

Run everything from `environment-controller/` once it exists.
